...
RESERVED_TEMPLATES = ['base.html', 'index.html', 'albumpage.html',
...
def process_image(config):
    ...
    imgtitle = improc.getTitle(image.filename, imgname)
    ...
    image.small_size = improc.resizeAndSave(image.source_path,
                                            (int(image.target_small_size[0]),
                                             int(image.target_small_size[1])),
                                            img_output_path, thumbnail=True)
    ...
    image.large_size = improc.resizeAndSave(image.source_path,
                                            (int(image.target_large_size[0]),
                                             int(image.target_large_size[1])),
                                            img_output_path)
    return image
...
class MediaObject:
    ...
    def __init__(self, type=TYPE_PHOTO, thumb_path=None, source_path="",
                 filename="", thumb_filename="", output_path=""):
        self.id = uuid.uuid4()
        self.type = type
    ...
    def __repr__(self):
        if self.type == MediaObject.TYPE_PHOTO:
            ...
            return "Video - %s, pre-transcoded: %s" % (self.filename,
                                                       self.transcoded)
