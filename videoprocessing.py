...
class VideoProcessor:
    ...
    def getScreencap(self, video_path, output_path):
        ...
        process = subprocess.Popen(
            capargs, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    def trancodeRawVideo(self, source_path, target_path, video_params,
        ...
        targetAspect = float(
            video_target_size[0]) / float(video_target_size[1])

        if sourceAspect > targetAspect:
            output_width = video_target_size[0]
            output_height = (
                float(video_target_size[0]) / float(video_params['width'])) * float(video_params['height'])
        else:
            output_height = video_target_size[1]
            output_width = (
                float(video_target_size[1]) / float(video_params['height'])) * float(video_params['width'])
        ...
        transcodeArgs = [self.getFFMpeg(),
                         "-y",
                         "-i",
                         source_path,
                         "-s",
                         "%sx%s" % (output_width, output_height),
                         "-r",
                         "25",
                         "-b",
                         "2M",
        ...
        process = subprocess.Popen(
            transcodeArgs, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    def getSizeAndDuration(self, filename):
        ...
        process = subprocess.Popen(
            [ffmpegPath,  '-i', filename],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT)
        stdout, stderr = process.communicate()

        matches = re.search(
            r"Duration:\s{1}(?P<hours>\d+?):(?P<minutes>\d+?):(?P<seconds>\d+\.\d+?),", stdout, re.DOTALL)
        if matches is None:
            raise Exception(
                "Couldn't get duration details for video %s" % filename)
        else:
            matches = matches.groupdict()

        matches.update(re.search(
            r"Video:.* (?P<width>\d+)x(?P<height>\d+)", stdout, re.DOTALL).groupdict())
